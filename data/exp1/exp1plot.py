...
x = range(10000,300000,10000)
fib_means = []
fib_err = []
heap_means = []
heap_err = []
with open('exp1fib_results.csv','rt') as f:
    for line in f:
        f = line.strip()
        row = line.split(',')
...
with open('exp1heap_results.csv','rt') as f:
    for line in f:
        f = line.strip()
        row = line.split(',')
        data = [int(i) for i in row]
...
basic_plot(['dijkstra fib','dijkstra heap'],data,
    'comparison of heap and fibonacci heap run time with graph sizes 10,000 to 290,000 with p_edge = 0.0001','graph size')
