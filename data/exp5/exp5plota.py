...
heap_means = []
heap_err = []
with open('exp5floyd_results.csv','rt') as f:
    for line in f:
        f = line.strip()
        row = line.split(',')
...
with open('exp5johnson_results.csv','rt') as f:
    for line in f:
        f = line.strip()
        row = line.split(',')
        data = [int(i) for i in row]
...
basic_plot(['floyd','johnson'],data,
    'comparison of floyd and johnson(returning distances) with 2000 nodes','number of edges')
