...
heap_means = []
heap_err = []
with open('exp6afloyd_results.csv','rt') as f:
    for line in f:
        f = line.strip()
        row = line.split(',')
...
with open('exp6ajohnson_results.csv','rt') as f:
    for line in f:
        f = line.strip()
        row = line.split(',')
        data = [int(i) for i in row]
...
basic_plot(['floyd','johnson'],data,
    'comparison of floyd and johnson(returning distances) with varying node sizes fixed p_edge at .1','number of nodes')
